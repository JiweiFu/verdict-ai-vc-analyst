...
def prepare_messages(messages: list[Message], system_prompt: str) -> list[Message]:
    ...
    # ``_trim_messages`` returns LangChain ``BaseMessage`` items; the runtime
    # consumers (``dump_messages`` -> ``convert_to_openai_messages``) handle those
    # transparently, so cast to the declared return type rather than widen it
    # (which would only push the invariance error to call sites).

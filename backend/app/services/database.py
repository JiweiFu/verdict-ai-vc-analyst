...
class DatabaseService:
    ...
    def __init__(self):
        ...
        # De-spam guard for health_check: log a failure warning only once per
        # "down" episode, reset to False on the next successful check.
    ...
    async def health_check(self) -> bool:
        ...
        try:
            with Session(self.engine) as session:
                ...
                # Recovered: reset the guard so a future outage warns once more.
                self._health_failure_logged = False
                return True
        except Exception as e:
            # Log one clear warning per "down" episode; stay quiet on consecutive
            # failures (e.g. repeated /health polling) to avoid flooding logs.
            if not self._health_failure_logged:
                ...
                logger.debug("database_health_check_failed_repeat", error=str(e))
            return False

...
class LangGraphAgent:
    ...
    def __init__(self):
        ...
        # De-spam guard: log a Postgres-unavailable warning only once per "down"
        # episode, reset to False when the pool is created successfully.
        self._pool_failure_logged: bool = False
        logger.info(
    ...
    async def _get_connection_pool(self) -> Optional[PostgresConnPool]:
        ...
        if self._connection_pool is None:
            try:
                ...
                # Pool is healthy again: reset the guard so a future outage can warn once more.
                self._pool_failure_logged = False
                logger.info("connection_pool_created", max_size=max_size, environment=settings.ENVIRONMENT.value)
            except Exception as e:
                # Pool stays None on failure, so this path is retried on every call.
                # Log a single clear warning per "down" episode instead of flooding.
                if not self._pool_failure_logged:
                    logger.warning(
                        "postgres_unavailable_degraded",
                        host=settings.POSTGRES_HOST,
                        port=settings.POSTGRES_PORT,
                        error=str(e),
                        environment=settings.ENVIRONMENT.value,
                    )
                    self._pool_failure_logged = True
    ...
    async def create_graph(self) -> Optional[CompiledStateGraph]:
        ...
        if self._graph is None:
            try:
                ...
                # De-spam: when the pool is down, _get_connection_pool already emitted the
                # single "postgres_unavailable_degraded" warning, so suppress the duplicate
                # graph_creation_failed log on the same episode. Raise/return semantics unchanged.
                if not self._pool_failure_logged:
